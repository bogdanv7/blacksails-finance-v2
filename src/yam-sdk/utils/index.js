...
export const stake2 = async (yam, amount, account, onTxHash) => {
  const poolContract = yam.contracts.ycrv_pool
  let now = new Date().getTime() / 1000;
  // const gas = GAS_LIMIT.STAKING[tokenName.toUpperCase()] || GAS_LIMIT.STAKING.DEFAULT;
  const gas = GAS_LIMIT.STAKING.DEFAULT
  if (now >= 1597172400) {
...
export const unstake2 = async (yam, amount, account, onTxHash) => {
  const poolContract = yam.contracts.ycrv_pool
  let now = new Date().getTime() / 1000;
  if (now >= 1597172400) {
    return poolContract.methods
...
export const harvest2 = async (yam, account, onTxHash) => {
  const poolContract = yam.contracts.ycrv_pool
  let now = new Date().getTime() / 1000;
  if (now >= 1597172400) {
    return poolContract.methods
...
export const redeem2 = async (yam, account, onTxHash) => {
  const poolContract = yam.contracts.ycrv_pool
  let now = new Date().getTime() / 1000;
  if (now >= 1597172400) {
    return poolContract.methods
