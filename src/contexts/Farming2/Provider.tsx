...
import { ycrvUniLp as ycrvUniLpAddress } from 'constants/tokenAddresses'
...
const Provider: React.FC = ({ children }) => {
  ...
  const ycrvPoolAddress = yam ? yam.contracts.ycrv_pool.options.address : ''
  ...
  const fetchEarnedBalance = useCallback(async () => {
    ...
    const balance = await getEarned(yam, yam.contracts.ycrv_pool, account)
  ...
  const fetchStakedBalance = useCallback(async () => {
    if (!account || !yam) return
    const balance = await getStaked(yam, yam.contracts.ycrv_pool, account)
  ...
  const handleStake = useCallback(async (amount: string) => {
    if (!yam) return
    setConfirmTxModalIsOpen(true)
    await stake2(yam, amount, account, () => {
      setConfirmTxModalIsOpen(false)
      setIsStaking(true)
    })
    setIsStaking(false)
  ...
  const handleUnstake = useCallback(async (amount: string) => {
    if (!yam) return
    setConfirmTxModalIsOpen(true)
    await unstake2(yam, amount, account, () => {
      setConfirmTxModalIsOpen(false)
      setIsUnstaking(true)
    })
